...
export class MediaDiscoveryClass implements IMediaDiscovery {
  ...
  public async registerTitles() {
    ...
    const getTitlesRegistrationInfoPromises = Object.keys(list).map(async (titleKey) => {
      let info: any = {}
      if ((list[titleKey].type = 'tvshow'))
        info = await this.getTVShowRegistrationInfo(titleKey, list[titleKey])
  ...
  public async getEpisodeList(titleObj: any) {
    ...
    const merged = [].concat(...seasonsList) // squash all season files into a single array
